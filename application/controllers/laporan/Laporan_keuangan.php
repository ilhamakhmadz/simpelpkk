...
class laporan_keuangan extends Admin_Controller
{
    ...
    public function index()
    {
        $this->load->vars(array(
            'page_title' => 'Laporan Keuangan Asset Daerah',
            // 'page_icon' => '<a class="btn btn-primary" href="' . site_url('master/desa/add') . '"> <i class="fa fa-plus"></i> Tambah</a><br>',
            'ui_controller' => 'laporan_keuangan',
        ));
        $data['kecamatan'] = $this->user_model->get_kecamatan();
        $this->template
            ->set_js(bower_url('datatables/media/js/jquery.dataTables.min'))
            ->set_js('app/laporan/laporan_keuangan/index')
            ->build('laporan/laporan_keuangan/index',$data);
    }
    ...
    public function cetak_laporan($kec,$desa){
        $data['desa'] = $this->db->get_where('master_desa', array('Kd_Desa' => $desa))->row();
        $data['kec'] = $this->db->get_where('master_kecamatan', array('Kd_Kec' => $kec))->row();

        $data['cetak'] =  $this->inventaris_model->cetak_keuangan($kec,$desa);
        // $data['cetak'] =  $this->inventaris_model->cetak_mutasi_tanah($kec,$desa);
        // $this->load->view('laporan/laporan_asset/print',$data);
        $html = $this->load->view('laporan/laporan_keuangan/print',$data,TRUE);
        ...
        $mpdf->Output('Laporan Keuangan Daerah - '.date('d M Y').'.pdf','I');
