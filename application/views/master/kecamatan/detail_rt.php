                ...
                <tbody class="text-gray-600 fw-bold">
                    <?php $i=1; foreach($rt as $value):?>
                        <tr>
                            ...
                            <td><?= $value->rt; ?></td>
                            <td>
                                <a href="<?=site_url().'/master/kecamatan/detail_dasawisma/'.$value->Kd_Desa.'/'.$value->Kd_Dusun.'/'.$value->rw.'/'.$value->rt?>" class="btn btn-icon btn-light-facebook me-5 ">
                                ...
                                <?php if($this->session->userdata('level_id') == 1 || $this->session->userdata('level_id') == 2 || $this->session->userdata('level_id') == 3 || $this->session->userdata('level_id') == 4 || $this->session->userdata('level_id') == 5 || $this->session->userdata('level_id') == 6):?>
                                <?php if($value->username == null):?>
                                <a href="<?=site_url(). '/master/kecamatan/generate_user_rt/'.$value->Kd_Desa.'/'.$value->Kd_Dusun.'/'.$value->rw.'/'.$value->rt?>" class="btn btn-icon btn-light-instagram me-5 ">
                                ...
                                <?php endif;?>
                                <a onclick="deleteItem('<?= $value->id ?>', '<?= $value->Kd_Desa ?>', '<?= $value->Kd_Dusun ?>', '<?= $value->rw ?>')"   
