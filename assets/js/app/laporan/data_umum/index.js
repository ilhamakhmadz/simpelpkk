function cetakPdf($id) {
    ...
    // Buat opsi tahun dari 2020 sampai tahun saat ini
    let yearOptions = '';
    ...
    // Tambahkan dropdown tahun dalam pesan toastr
    toastr.info(`
        Pilih Tahun Pelaporan?<br /><br />
        <select id="yearSelect" class="form-control">
            ${yearOptions}
        </select>
        <a onclick='window.selectedYes(${$id}, document.getElementById("yearSelect").value)' 
        type='button' 
...
function selectedYes($id, $year) {
    ...
    // window.location = site_url + "laporan/data_umum/pdf/" + $id + "/" + $year;
    window.open(site_url + "laporan/data_umum/pdf/" + $id + "/" + $year, '_blank');
}
    ...
    the_table = $("#dataTable_profil_kecamatan").DataTable({
        ...
        "columns": [{
            ...
            { "data": "jml_penduduk" },
            {
                ...
                "orderable": false,
                "render": function(data, type, row, meta) {
                    return '<a href="' + site_url + 'laporan/data_umum/view/' + row.id + '" class="btn btn-icon btn-light-twitter me-5 ">\n\
